function notch_and_BR(sbj_name, project_name, bns, dirs)
...
for i = 1:length(bns)
    ...
    title(['Bad electrodes based on raw power (variance) ',bn])
    xlabel('Time (s)')
    ...
    %% Re-referencing data to the bipolar reference - and save
    clear data
    data_all = data_all';
    ...
    globalVar.channame_BR = channame_BR;
    globalVar.BR_idx = BR_idx;
    ...
    for bii=1:size(data_all,1)-1
        ...
        if strcmp(name1,name2)
            ...
            disp(['Writing: ' sprintf('%s/BRiEEG%s_%.2d.mat',globalVar.BRData,bn, el)])
        save(sprintf('%s/BRiEEG%s_%.2d.mat',globalVar.BRData,bn, el),'data');
        clear data
        else
            continue
        end

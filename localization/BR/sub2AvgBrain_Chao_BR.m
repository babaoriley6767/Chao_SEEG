function [avgCoords, elecInfo_table, elecCoord, avgVids, subVids]=sub2AvgBrain_Chao_BR(cfg, dirs, sbj_name, fsDir_local)
...
if isempty(elecNames)
    ...
    elecFname=fullfile(subDir,'elec_recon_BR',[subj '.electrodeNames_BR']);
...
if isempty(elecCoord) % no electrode coordinates have been passed in the function call:
    ...
    coordFname=fullfile(subDir,'elec_recon_BR',[subj '.LEPTO_BR']);
...
if universalYes(rmDepths),
    ...
    [avgCoordsDepths, elecNamesDepths]=depths2AvgBrain_Chao_BR(dirs);% 只计算depth
end
...
for hemLoop=1:2,
    ...
    if ~isempty(hemElecIds)
        ...
        for a=1:nHemElec,
            if isSubdural(hemElecIds(a)),
                df=avg_sph.vert-repmat(sph.vert(subVids(hemElecIds(a)),:),nAvgVert,1);%这里计算subdural
                dst=sum(df.^2,2);
...
%             elecNamesDepths(b) = append(elecInfo_table.Hem(b),elecInfo_table.Name(b));%%% is this wrong?
%         end 
        
        for a=1:nHemElec,
            if isSubdural(hemElecIds(a)),
                avgCoords(hemElecIds(a),:)=avgPial.vert(avgVids(hemElecIds(a)),:);%chao this is the subdural average coords%这里计算subdural
            else
...
elecInfo_table.elecNames = elecNames;%%%output of this function
